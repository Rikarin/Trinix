...
abstract final class Log {
    ...
    static void Print(Args...)(Args args) if (args.length >= 1) {
        ...
        } else static if (IsIterable!S && is (S == char[])) {
            Print(args[0][0]);
    ...
    static void Emergency(string file = __FILE__, string func = __PRETTY_FUNCTION__, int line = __LINE__, Args...)(Args args) {
        opCall!(file, func, line)(LogLevel.Emergency, args);
    }

    static void Critical(string file = __FILE__, string func = __PRETTY_FUNCTION__, int line = __LINE__, Args...)(Args args) {
        opCall!(file, func, line)(LogLevel.Critical, args);
    }

    static void Error(string file = __FILE__, string func = __PRETTY_FUNCTION__, int line = __LINE__, Args...)(Args args) {
        opCall!(file, func, line)(LogLevel.Error, args);
    }

    static void Alert(string file = __FILE__, string func = __PRETTY_FUNCTION__, int line = __LINE__, Args...)(Args args) {
        opCall!(file, func, line)(LogLevel.Alert, args);
    }

    static void Warning(string file = __FILE__, string func = __PRETTY_FUNCTION__, int line = __LINE__, Args...)(Args args) {
        opCall!(file, func, line)(LogLevel.Warning, args);
    }

    static void Notice(string file = __FILE__, string func = __PRETTY_FUNCTION__, int line = __LINE__, Args...)(Args args) {
        opCall!(file, func, line)(LogLevel.Notice, args);
    }

    static void Info(string file = __FILE__, string func = __PRETTY_FUNCTION__, int line = __LINE__, Args...)(Args args) {
        opCall!(file, func, line)(LogLevel.Info, args);
    }

    static void Debug(string file = __FILE__, string func = __PRETTY_FUNCTION__, int line = __LINE__, Args...)(Args args) {
        opCall!(file, func, line)(LogLevel.Debug, args);
    }
    ...
    private static void Scroll() {
        if (m_y == Height || (m_y == Height - 1 && m_x == Width)) { /* If we are on end of terminal */
            ...
            m_display[(m_y - 1) * Width .. Width * Height].Address = 0;
            m_y--;
        }
    }

    private static void Put(const(char) c, byte color = 0x7) {
        const(char[1]) c;
        c[0] = c;
        Put(c, color);
    }

    private static void Put(const(char[]) str, byte color = 0x7) {
        Serial1.Write(str);
